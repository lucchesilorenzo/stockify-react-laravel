...
class DashboardController extends Controller
{
	...
	public function getLowStockProducts(): JsonResponse
	{
		try {
			...
			$lowStockProducts = count($products->filter(function ($product) {
				return $product->quantity <= 10;
			}));

			return response()->json($lowStockProducts);
		} catch (\Throwable $e) {
			return response()->json(['message' => 'Failed to get low stock products.'], 500);
		}
	}
	...
	public function getShippedOrders(): JsonResponse
	{
		try {
			...
			return response()->json(['message' => 'Failed to get shipped orders.'], 500);
		}
	}
	...
	public function getUnitsInStock(): JsonResponse
	{
		try {
			...
			return response()->json(['message' => 'Failed to get units in stock.'], 500);
		}
	}
